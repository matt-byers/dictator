...
class Recorder:
    ...
    def stop(self): self.stop_count += 1; return self.audio
    def close(self): pass


class Transcriber:
    def __init__(self): self.count = 0
    def transcribe(self, _audio): self.count += 1; return "Howdy."


class Inserter:
    def __init__(self): self.values = []
    def insert(self, text, _keys_released): self.values.append(text)


class View:
    def __init__(self): self.states = []; self.errors = []
    def show_state(self, state): self.states.append(state)
    def show_error(self, error): self.errors.append(error)


class ControllerTests(unittest.TestCase):
    def setUp(self):
        self.recorder, self.transcriber, self.inserter, self.view = Recorder(), Transcriber(), Inserter(), View()
        self.controller = DictationController(
            self.recorder, self.transcriber, self.inserter, self.view,
            lambda: True, logging.getLogger("test"), thread_factory=ImmediateThread,
        )
    ...
    def test_complete_dictation_is_ordered_and_returns_idle(self):
        ...
        self.assertEqual(
            self.view.states,
            [SessionState.OPENING, SessionState.RECORDING, SessionState.TRANSCRIBING, SessionState.PASTING, SessionState.IDLE],
        )
